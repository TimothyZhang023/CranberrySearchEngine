...
public class RedisSafe {
    ...
    public RedisSafe() {
        if (null == pool) {
            synchronized (RedisSafe.class) {
                if (null == pool) {
                    RedisPoolClient redisPoolClient = new RedisPoolClient(
                            ConfigurationUtil.getValue("RedisServerIp", "127.0.0.1"),
                            Integer.parseInt(ConfigurationUtil.getValue("RedisServerPort", "6379")),
                            Integer.parseInt(ConfigurationUtil.getValue("RedisServerPoolSize", "50000"))
                    );
                    pool = redisPoolClient.getJedisPool();
                }
            }
        }
     ...
     * @param port 端口
     */
    public RedisSafe(String ip, int port) {
        if (pool == null) {
...
//     * @param ip       ip
//     * @param port     端口
//     * @param poolSize 资源池大小
//     */
//    public RedisSafe(String ip, int port, int poolSize) {
//        if (pool == null) {
//            RedisPoolClient redisPoolClient = new RedisPoolClient(ip, port, poolSize);
//            pool = redisPoolClient.getJedisPool();
//        }
//    }
